...
for seed in SEEDS:
    for checkpoint in SAVE_STEPS_SCHEDULE:
        this_commands = [
            ...
            f"--do_predict "
            f"--test_file /SAN/intelsys/rclearn/when-do-reading-comprehension-models-learn/data/processed/squad1_dadversarialQA_1_1_weighted_train.json "
            f"--per_device_eval_batch_size 64 "
            f"--output_dir /SAN/intelsys/rclearn/when-do-reading-comprehension-models-learn/predictions/on-train/albert-xlarge-v2-squadv1-adversarialall-wu=100-lr=3e5-bs=32-msl=384-seed={seed}/checkpoint-{checkpoint} "
            f"--overwrite_output_dir "
            f"--overwrite_cache "
            f"--report_to none "
            f"> logs/on-train/albert-xlarge-v2-squadv1-adversarialall-wu=100-lr=3e5-bs=32-msl=384-seed={seed}-checkpoint={checkpoint}.log 2>&1"
        ]
...
"""
    ...
    # Save the job file
    with open(f'jobs/eval/albert-xlarge-v2-squadv1-adversarialall-eval-on-train.job', 'w') as f:
        f.write(job_file)
